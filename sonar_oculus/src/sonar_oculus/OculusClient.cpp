...
void OsBufferEntry::ProcessRaw(char* pData) {
  ...
  switch (head.msgId) {
    case messageSimplePingResult: {
      ...
      if (m_rfm.fireMessage.head.payloadSize + sizeof(OculusMessageHeader) ==
          m_rfm.imageOffset + m_rfm.imageSize) {
        ...
        if (m_pBrgs)
          memcpy(m_pBrgs, pData + sizeof(OculusSimplePingResult),
                 m_rfm.nBeams * sizeof(short));
      } else
        std::cerr << "Error in Simple Return Fire Message. Byte Match:" +
                         std::to_string(m_rfm.fireMessage.head.payloadSize +
                                        sizeof(OculusMessageHeader)) +
                         " != " +
                         std::to_string(m_rfm.imageOffset + m_rfm.imageSize);
    } break;
  }
  m_mutex.unlock();
}
...
OsReadThread::~OsReadThread() {
  if (m_pRxBuffer) delete m_pRxBuffer;
...
// ----------------------------------------------------------------------------
// If we are running then switch off the running flag and wait for the thread to
// exit
...
void OsReadThread::ProcessRxBuffer() {
  ...
  if (m_nRxIn >= (int64_t)sizeof(OculusMessageHeader)) {
    ...
    // It might be possible to try and find a vlid header by searching for the
    // correct id here
    if (pOmh->oculusId != 0x4f53) {
      m_nFlushes++;
      std::cerr << "Having to flush buffer, unrecognised data. #:" +
                       std::to_string(m_nFlushes);
      m_nRxIn = 0;
      return;
    }
...
void OsReadThread::run() {
  ...
  /* Do not want run to open sockets
    ...
    std::to_string(m_port) + " Reason:" + m_pSocket->errorString();
  ...
  */
  ...
  if (IsActive()) {
    ...
    if (bytesAvailable > 0) {
      ...
      unsigned bytesRead =
          read(*m_pSocket, (char*)&m_pRxBuffer[m_nRxIn], bytesAvailable);
...
OsClientCtrl::~OsClientCtrl() { m_readData.Shutdown(); }
...
bool OsClientCtrl::IsOpen() { return (m_readData.IsActive()); }
...
void OsClientCtrl::Fire(int mode, double range, double gain,
                        double speedOfSound, double salinity) {
  if (IsOpen()) {
    ...
    if (false) flags |= 0x10;  // flagsGainAssist;
